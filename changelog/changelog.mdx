...
<Update label="v1.0.6">
  ...
  - **New `SlumberChunker`**: Welcome Chonkie's very own agentic chunker! Requires the `genie` optional install (`pip install "chonkie[genie]"`) and a `GEMINI_API_KEY`. It leverages `Genie`, Chonkie's interface for generative models.
  ...
  - **New `NeuralChunker`**: Introducing a fully neural approach to chunking! Requires the `neural` optional install (`pip install "chonkie[neural]"`). This uses a fine-tuned BERT-like model for fast, high-quality chunking.
