...
chunker = NeuralChunker(
    model="mirth/chonky_modernbert_large_1", # Default model
...
NeuralChunker returns chunks as `NeuralChunk` objects:
...
@dataclass
class NeuralChunk(Chunk):
    text: str           # The chunk text
    start_index: int    # Starting position in original text
    end_index: int      # Ending position in original text
    token_count: Optional[int] = None # Token count might require separate calculation
