...
<Info> In `RecursiveLevel`, it is expected that only delimiters are provided, without whitespace as a delimiter.
 If whitespace is needed as a delimiter, you can use the `whitespace` parameter in the `RecursiveLevel` class, 
 by setting it to `True`. Though, you cannot pass delimiters in this case.</Info>
