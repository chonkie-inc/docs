...
SlumberChunker returns chunks as `SlumberChunk` objects, potentially with extra metadata from Genie:

```python
from dataclasses import dataclass
from chonkie.chunkers.base import Chunk # Assuming base Chunk structure

@dataclass
class SlumberChunk(Chunk):
    text: str           # The chunk text
    start_index: int    # Starting position in original text
    end_index: int      # Ending position in original text
    token_count: int    # Number of tokens in chunk
    # Example extra field:
    genie_rationale: Optional[str] = None # Why Genie made this split (if verbose)
    # Add any other specific fields provided by Genie
```
